...
class Sender(object):
    def __init__(self, receiver, unique_id="csse120", who_am_i=1):
        if who_am_i == 1:
            self.sender = SenderReceiver(unique_id, "one", "two", receiver)
        else:
            self.sender = SenderReceiver(unique_id, "two", "one", receiver)
